...
#some constants
orf_start <- 423 # should be in the ref seq name? - this is for nanoluc set
tag_offset <- 44 -14 #  number of codons of the tag compensated by our A1 start site so that SynGAP p. are correct
Macrogen_insert_pos <-288+14*3+1 # with reference to start of SynGAP, in bases 
...
#***The first entry should be the reference***, the wild type plasmid sequence as it should be
...
rotate_to_match <- function(seq, ref, window_start = 1, window_size = 50) {
  ...
  for (orientation in c("forward", "reverse")) {
    ...
    # # currently we are not set up for searches across boundaries - fix this when needed
    # # Wrap the sequence because our ref_window could be at a boundary!
    # wrapped_seq <- paste0(seq_str, seq_str, seq_str)
    # L <- nchar(seq_str)
    # 
    # #Find match position
    # match_pos <- regexpr(ref_window, wrapped_seq)[1]
    # match_pos
...
#Remove rows where any character column starts with ";"
seq_list <- seq_list[!apply(seq_list, 1, function(row) any(grepl("^;", row))), ]
...
for (i in 2:nrow(seq_list)) {
  ...
  seq_rot <- rotate_to_match(seq, ref=ref_seq, window_start = orf_start)# searching the tag...+tag_offset*3) # get match around start site otherwise the sample read will be out with an indel
...
align_filename <-paste0(input_path, "custom_clustal_alignment.aln")
...
align_aa_filename <-file.path(input_path, "custom_clustal_alignment_aa.aln")
